...
void run_experiment(complex_t *x, int n,
  ...
  int filter_freq_size = filter.sizet;
  ...
  #pragma acc data copyin(x[0:n]) copyout(cufft_x_f[0:n])
  {
    for(int i = 0; i < repetitions; i++){
      #pragma acc host_data use_device(x,cufft_x_f) 
      err = cufftExecZ2Z(plan, (cufftDoubleComplex *)x, (cufftDoubleComplex *)cufft_x_f, CUFFT_FORWARD);
    }
  }
  
  printf("Time to run cuFFT : %lf\n", get_time());
...
int main(int argc, char **argv){
  ...
  //Dummy data region for OpenACC runtime initialization
  int a[10];
